...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        ...
        if exists(self.mlp) and exists(time_emb):
            ...
            time_emb = time_emb[..., None, None, None]   # "b c -> b c 1 1 1"
            h = h + time_emb
...
class ConvNextBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        ...
        if exists(self.mlp) and exists(time_emb):
            ...
            h = h + condition[..., None, None]           # "b c -> b c 1 1"
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # "b (h c) x y z -> b h c (x y z)": split channels into heads, flatten spatial
        q, k, v = (t.reshape(t.shape[0], self.heads, -1, t.shape[2] * t.shape[3] * t.shape[4])
                   for t in qkv)
        q = q * self.scale
        ...
        # "b h (x y z) d -> b (h d) x y z": (b,heads,xyz,d) -> (b,heads,d,xyz) -> (b,heads*d,l,h,w)
        out = out.permute(0, 1, 3, 2).contiguous().reshape(b, -1, l, h, w)
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # "b h c (x y z) -> b (h c) x y z": (b,heads,c,xyz) -> (b,heads*c,l,h,w)
        out = out.reshape(b, -1, l, h, w)
        return self.to_out(out)
